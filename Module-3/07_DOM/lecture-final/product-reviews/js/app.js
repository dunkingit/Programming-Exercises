const bookName ='Hype button Parties for Smarties';
...
const span = h1.querySelector('.name');
...
function setPageDescription() {
 const pTag = document.getElementById('main').querySelector('.description');
...
function displayReviews() {
  ...
  reviews.forEach((review) => {
    //make inner review container
    const reviewContainer = document.createElement('div');
    reviewContainer.setAttribute('class', 'review')
    //attach each part to review container
  ...
  //append the review container to main element
...
function addRating(parent, numberOfStars) {
  //first make the rating div container and set its attributes
  const ratingDiv = document.createElement('div');
  ratingDiv.setAttribute('class', 'rating');

  //loop based off of the number of stars passed in so that we will create that many img tags
  for(let i = 0; i < numberOfStars; i++){
    //creating an img tag
    const starImage = document.createElement('img');
    //setting the src value for that img tag
    starImage.setAttribute('src', './img/star.png');
    //now that we have an img tag we are adding it to the ratingDiv element from above
