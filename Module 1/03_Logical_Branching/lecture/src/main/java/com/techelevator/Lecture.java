...
//            return "Minor";
//        }
        return number >= 18? "Adult": number >= 13? "Teen":"Minor";
    }
    ...
    public double returnPizzaCost(char size, int numberOfToppings) {
        double amount = size == 's'? SMALL:size == 'm'? MEDIUM:LARGE;
...
//        return amount + (numberOfToppings + (numberOfToppings <= 3? 1.00:0.75));
        double per_Topping = numberOfToppings <= 3? UNDER_3_TOPPINGS:OVER_3_TOPPINGS;
        System.out.println(amount);
        System.out.println(per_Topping);
        return amount + (numberOfToppings * per_Topping);
