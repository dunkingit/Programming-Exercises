...
public class Exercises {
	...
	public boolean hasBad(String str) {
		return (str.length() > 1) && (str.startsWith("bad") || str.startsWith(str.charAt(0) + "bad"));
	}
	...
	public String frontTimes(String str, int n) {
		return n >= 0 && str.length() > 2? str.substring(0, 3).repeat(n):str.repeat(n);
	}
	...
	public int last2(String str) {
		...
		String newWholeString = str.substring(0, str.length() - 1);
		String lastTwoLetters = str.substring(str.length() - 2);
		int counter = 0;
		for (int i = 0; i < newWholeString.length() - 1; i++) {
			if (i + 2 > newWholeString.length()){
				String temp = str.substring(i, i + 1);
				if(temp.equals(lastTwoLetters)){
					counter++;
				};
				break;
			}
			String temp = newWholeString.substring(i, i + 2);
			if(temp.equals(lastTwoLetters)){
				counter++;
			};
		}
	...
	public String stringX(String str) {
		...
		return str.substring(start, end + 1).replaceAll("x", "");
	}
	...
	public String altPairs(String str) {
		String temp = "";
		int[] intArray = {0,1,4,5,8,9,12,13};
		for(int each: intArray){
			try{
				temp += str.charAt(each);
			}catch(Exception ex){
				return temp;
			}
		}
