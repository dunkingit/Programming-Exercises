...
public class Lecture {
    ...
    */
    ...
    public double returnPizzaCost(char size, int numberOfToppings) {
        ...
        double totalPizzaCost = 0.00;

        if(size == 's'){
            totalPizzaCost = totalPizzaCost + SMALL;
        } else if(size == 'm'){
            totalPizzaCost += MEDIUM;
        } else {
            totalPizzaCost += LARGE;
        }

       if(numberOfToppings <= 3){
          totalPizzaCost = totalPizzaCost + (numberOfToppings * UNDER_3_TOPPINGS);
       } else{
           totalPizzaCost += (numberOfToppings * OVER_3_TOPPINGS);
       }
